...
const InstaFeed = () => {
        ...
        if(item.media_type === "IMAGE"){
            return(
                <a key={item.id} href={item.permalink} target="_blank" rel="noreferrer" className="item rounded overflow-hidden transition duration-400 ease-in-out hover:brightness-130">
                    <img src={item.media_url} style={{ width: '100%' }} alt={item.caption} />
                </a>
                );
        } 
    ...
    const thumbnailTemplate = (item) => {
        if (item.media_type === "VIDEO") {
            return <img src={item.thumbnail_url} className="w-28 h-28" alt={item.caption} />;
        } else {
            return <img src={item.media_url} style={{ width: '90%' }} alt={item.caption} />;
        }
    };
